...
async def on_message(message):
    ...
    if not message.content.startswith('.') and (message.author.id in bot_id or message.author.id == user_id) and public_chat_num != 0:
        if not isinstance(message.channel, discord.DMChannel):
            ...
            vals_save('vals.json', 'channel_id', channel_id)
            if message.content:
    ...
    if not message.content.startswith('!') and message.author.id == user_id:
        ...
        if isinstance(message.channel, discord.DMChannel):
            ...
            vals_save('vals.json', 'dm_channel_id', dm_channel_id)
...
async def chatlog(interaction: discord.Interaction):
    ...
    if interaction.user.id == user_id:
        ...
        vals_save('vals.json', 'console_log', console_log)
    else:
        randaw = noperm_answ()
        await interaction.response.send_message(f"`{randaw}`", ephemeral=True)
...
async def emo_rate(interaction: discord.Interaction, rate: int):
    ...
    if interaction.user.id == user_id:
        ...
        if 0 <= rate <= 100:
            ...
            vals_save('vals.json', 'emoji_rate', emoji_rate)
        else:
...
async def public_bot_chat(interaction: discord.Interaction, limit: int = None):
    ...
    if interaction.user.id == user_id:
        ...
        vals_save('vals.json', 'public_chat_num', public_chat_num)
        vals_save('vals.json', 'public_chat', public_chat)
    else:
        randaw = noperm_answ()
        await interaction.response.send_message(f"`{randaw}`", ephemeral=True)
...
async def member_info():
    ...
    vals_save('vals.json', 'user_nick', user_nick)
...
async def count_msg():
    ...
    vals_save('vals.json', 'total_msg', total_msg)
...
async def bot_idle():
    ...
    await mood_drop()
    public_chat_num = vals_load('vals.json', 'public_chat_num')
